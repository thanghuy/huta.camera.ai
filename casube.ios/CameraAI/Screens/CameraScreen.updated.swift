...
/// 
/// UPDATED FOR WIN-9: Auto-capture with PerfectStateTracker
/// - Tracks perfect state dwell time (500ms)
/// - Auto-captures after dwell
/// - Enforces cooldown (2s) between captures
/// - Different haptic feedback for auto vs manual
struct CameraScreen: View {
    ...
    @State private var autoCaptureSubscription: AnyCancellable?
    ...
    var body: some View {
        ...
        .onAppear { setupAutoCapture() }  // WIN-9: Setup auto-capture listener
        .onDisappear {
            controller.stop()
            autoCaptureSubscription?.cancel()  // WIN-9: Cleanup subscription
        }
        ...
        .onChange(of: store.mode) { _, newMode in
            ...
            store.resetAutoCapture()  // WIN-9: Reset tracker on mode change
        }
        .onChange(of: store.selectedPoseID) { _, _ in
            store.resetAutoCapture()  // WIN-9: Reset tracker on pose selection
        }
        .animation(.easeInOut(duration: 0.18), value: store.showSavedToast)
    }

    // MARK: - Auto-Capture Setup (WIN-9)

    private func setupAutoCapture() {
        autoCaptureSubscription = store.autoCaptureTracker.$shouldCapture
            .filter { $0 }
    ...
    private var actionRow: some View {
        HStack {
            ...
            // WIN-9: ShutterButton respects cooldown
    ...
    private func startCamera() async {
        ...
        controller.onFrame = { pixelBuffer, orientation in
            ...
            DispatchQueue.main.async {
                ...
                // WIN-9: Update auto-capture tracker with match state
    ...
    // MARK: - Capture Methods (WIN-9: Auto vs Manual)

    /// Auto-capture triggered by dwell time (stronger haptic)
    private func capturePhotoForAutoCapture() {
        UIImpactFeedbackGenerator(style: .heavy).impactOccurred()
        performPhotoCapture()
    }

    /// Manual capture via shutter button (normal haptic)
    private func capturePhotoManual() {
        UIImpactFeedbackGenerator(style: .medium).impactOccurred()
        performPhotoCapture()
    }

    /// Shared photo capture logic (used by both auto and manual)
